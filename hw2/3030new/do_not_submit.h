...
int getDelay() { return delay_n; }

void setSleeperN(int d) {
  if (d >= 0) sleeper_n = d;
}

int getSleeperN() { return sleeper_n; }
...
void drawWindow() {
  if (COLS > 90 && LINES > 40) {
    ...
    double elapsed = (time_now.tv_sec - time_pre.tv_sec) * 1e3 +
                     (time_now.tv_nsec - time_pre.tv_nsec) / 1.0e6;
    time_pre = time_now;
    ...
    mvprintw(
        0, 0,
        "Elapsed time since last call to drawWindow(): %5.5f               ",
        elapsed);
    mvprintw(1, 0, "Total number of actions per ms: %f               ",
             n_actions != 0 ? n_actions / elapsed : 0);
    mvprintw(2, 0,
             "# Ants(sleep/total): (%3d/%3d) |# Foods: %3d |# Threads: %s",
             nsants, nants, nfoods, thr);
    mvprintw(3, 0, "Expected number of sleepers: %3d, Delay amount: %3d",
             sleeper_n, delay_n);
    mvprintw(LINES - 2, 0,
             "'q' for exit, '+' and '-' for delay, '*' and '/' for sleepers.");
